        ...
        try:
            ...
            for i in a:
                if 'href' in i.attrs:       #判断页面中有没有url
                    with open(fr"C:\Users\Administrator\Desktop\目录\{i.text}.txt", "a", encoding='utf-8') as f:
                        f.write(i.text + ":" + i.attrs['href'] + "\n")      #如果有就将url和标签名写入txt中
                    if 'http' in i.attrs['href'] and ('.com' in i.attrs['href'] or '.net' in i.attrs['href'] or '.cn' in i.attrs['href']):     #判断这个URL是否包括http和.com，因为requests只能解析带http的URL
